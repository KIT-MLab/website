/**
 * 先生の画面が読む口の土台（20-platform.md 第5.4節・第8章）。
 *
 * ここに置くのは2つだけ。**入ってよいかどうか**（ロール）と、**誰の記録を見てよいか**
 * （範囲）である。ルート（src/pages/api/teacher/*.ts）はこの2つを並べるだけで済ませ、
...
/**
 * 先生以上であることを確かめる。合わなければ null（例外にしない）。
 *
 * 入っていない人も、学生も、同じ null になる。**どちらなのかを呼ぶ側に伝えない。**
 * 先生の画面は学生には存在しない画面なので、断り方を分ける意味がない（第8章）。
 */
export async function requireTeacher(request: Request): Promise<CurrentUser | null> {
