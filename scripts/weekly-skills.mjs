...
export const SKILLS = [
  ...
  { key: 'index-len', chapter: '04-loop', name: '添字と len() で位置を指定する', re: /\blen\s*\(|\b[a-z_]\w*\[\s*-?\w+\s*\]/ },
  ...
  { key: 'default-arg', chapter: '05-function', name: '既定値のある引数', re: /def\s+\w+\s*\([^)]*=[^)]*\)/ },
  ...
  { key: 'np-mask', chapter: '07-array', name: '条件で取り出す（配列[条件]）', re: /\w\[[^\]\n]*(<|>|==|!=)[^\]\n]*\]/ },
  { key: 'np-slice', chapter: '07-array', name: 'スライスで範囲を取り出す', re: /\[[^\]\n]*:[^\]\n]*\]/ },
  // 第8章
  { key: 'np-ij', chapter: '08-table', name: '行と列の添字 [i, j]・[:, j]', re: /\w\[[^\[\]\n]*,[^\[\]\n]*\]/ },
