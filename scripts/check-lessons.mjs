...
for (const file of files) {
  ...
  /* --- 検査15 節ごとに「組む」課題があること（第3.8節） ---
     ...
     **章の合計ではなく、節ごとに見る。** 合計で見ていたとき、「組む」を置けない節
     （第5.4節 スコープ）のぶんを別の節が2問持ち、その2問目は1問目に含まれるもの
  ...
  for (const e of builds) {
    ...
    // テストケースは <Exercise> が tests から組んで出す（10-lesson 第3.1節）。
    // 手で書くと隠しテストとずれても誰も気づかないので、問題文には書かせない。
    if (e.prompt.includes('テストケース')) {
      add(5, e.line, '問題文に「テストケース」を手で書いています。tests から機械が組んで出すので消してください');
    }
  }
...
/* --- 検査18 まだ出てきていない語を地の文で使っていないこと ---
   ...
   インデント・型・クリック・デスクトップのような語は、書き手には当たり前すぎて
